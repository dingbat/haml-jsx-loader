...
function replaceMatching(string, regex, open, close, replacement) {
  var newString = "";

  var counter = 0;

  do {
    var match = string.substring(counter).match(regex);
    if (!match) {
      break;
    }

    var head = string.substring(counter, counter + match.index);
